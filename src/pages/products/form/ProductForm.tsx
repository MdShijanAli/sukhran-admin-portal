  ...
  const fetchProductDetails = async (productId: string) => {
    try {
      ...
      if (product.categoryId) {
        handleCategoryChange(product.categoryId.toString());
      }
    } catch (error) {
    ...
    try {
      ...
      skus.forEach((sku, index) => {
        Object.keys(sku).forEach((key) => {
          ...
          if (value !== undefined && value !== "" && key !== "imgUrl") {
            formDataToSubmit.append(`skus[${index}][${key}]`, value.toString());
          }
        });

        // Add SKU image if exists
        if (sku.imgUrl && sku.imgUrl instanceof File) {
          formDataToSubmit.append(`skus[${index}][imgUrl]`, sku.imgUrl);
        }
      });
                ...
                <div className="grid gap-4 md:grid-cols-3">
                  ...
                  <div className="grid gap-2">
                    ...
                    <Input
                      ...
                      onChange={(e) =>
                        updateSku(index, "currentPrice", Number(e.target.value))
                      }
                      required
                    />
                  </div>
