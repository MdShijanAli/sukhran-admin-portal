      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        <Card>
          <CardContent className="p-4">
            ...
            <p className="text-2xl  mt-2">{analytics.views.toLocaleString()}</p>
            <p className="text-xs text-muted-foreground">Total Views</p>
            <p
              className={cn(
                "text-xs mt-1",
                getTrendColor(analytics.viewsTrend),
              )}
            >
              {analytics.viewsTrend > 0 ? "+" : ""}
              {analytics.viewsTrend}% vs last week
            </p>
          </CardContent>
        </Card>
        ...
        <Card>
          <CardContent className="p-4">
            ...
            <p
              className={cn(
                "text-xs mt-1",
                getTrendColor(analytics.clicksTrend),
              )}
            >
              {analytics.clicksTrend > 0 ? "+" : ""}
              {analytics.clicksTrend}% vs last week
            </p>
          </CardContent>
        </Card>
        ...
        <Card>
          <CardContent className="p-4">
            <div className="flex items-center justify-between">
              ...
              <Badge
                variant={Number(engagementRate) > 5 ? "default" : "secondary"}
              >
                {Number(engagementRate) > 5 ? "Good" : "Low"}
              </Badge>
            </div>
            <p className="text-2xl  mt-2">{engagementRate}%</p>
            <p className="text-xs text-muted-foreground">Engagement Rate</p>
            <Progress
              value={Math.min(Number(engagementRate) * 5, 100)}
              className="mt-2 h-1"
            />
          </CardContent>
        </Card>
        ...
        <Card>
          <CardContent className="p-4">
            ...
            <p className="text-2xl  mt-2">{avgTimeFormatted}</p>
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4 text-sm">
            ...
            <div>
              ...
              <p className="font-medium">
                {content.publishDate || "Not published"}
              </p>
            </div>
            <div>
